...
@interface CountdownDetailOptionsViewController ()

@end

@implementation CountdownDetailOptionsViewController
...
- (void)dismissDatePicker:(id)sender {
	...
	if ([self changeButton] == 0) {
		if ([[[self currentCountdown] date] compare:[NSDate dateWithTimeInterval:[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:[[self datePickerPointer] date]]-[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:realdate] sinceDate:[[self datePickerPointer] date]]] != NSOrderedSame) {
			NSDateComponents *comps = [calendar components:NSMaxCalendarUnit fromDate:today toDate:[[self datePickerPointer] date] options:0];
			if ([comps year] <= 100 || [[[[self currentCountdown] timeset] self] invincible]) {
				[self finishDateSetting1];
			}
			else {
				UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"Timesets may not be applied to countdowns with more than 100 years remaining until their target dates. Remove timeset from this countdown?" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Remove Timeset" otherButtonTitles:nil];
				[actionSheet showInView:[self view]];
			}
		}
	}
	else {
		if ([[[[self currentCountdown] elapsed] date] compare:[NSDate dateWithTimeInterval:[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:[[self datePickerPointer] date]]-[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:realdate] sinceDate:[[self datePickerPointer] date]]] != NSOrderedSame) {
...
- (void)finishDateSetting1 {
	...
	[self updateUI];
	[[self currentCountdown] setupCountdown];
}

- (void)finishDateSetting2 {
	[[[self currentCountdown] elapsed] setDate:[NSDate dateWithTimeInterval:[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:[[self datePickerPointer] date]]-[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:realdate] sinceDate:[[self datePickerPointer] date]]];
	[[self timesetButton] setTitle:[NSString stringWithFormat:@"Timeset: %@", [[[self currentCountdown] timeset] name]] forState:UIControlStateNormal];
	[self updateUI];
	[[self currentCountdown] setupCountdown];
}
...
- (IBAction)callDP:(id)sender {
	[self setChangeButton:[[sender restorationIdentifier] intValue]];
	...
	if ([self changeButton] == 0) {
		...
		[datePicker setDate:[NSDate dateWithTimeInterval:[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:realdate]-[[NSTimeZone systemTimeZone] daylightSavingTimeOffsetForDate:[[[self currentCountdown] elapsed] date]] sinceDate:[[[self currentCountdown] elapsed] date]]];
...
- (IBAction)switchTimeset:(id)sender {
	...
	NSDateComponents *comps = [calendar components:NSMaxCalendarUnit fromDate:today toDate:[[self currentCountdown] date] options:0];
	NSDateComponents *comps2 = [calendar components:NSMaxCalendarUnit fromDate:[[[self currentCountdown] elapsed] date] toDate:today options:0];
	if ([comps year] > 100) {
		actionSheet = [[UIActionSheet alloc] initWithTitle:@"Timesets may not be applied to countdowns with more than 100 years remaining until their target dates." delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:nil];
	}
	else if ([comps2 year] > 100) {
		actionSheet = [[UIActionSheet alloc] initWithTitle:@"Timesets may not be applied to countdowns with more than 100 years elapsed since their start dates." delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:nil];
	}
	else {
...
- (IBAction)switchUnits:(id)sender {
	...
	for (int i=0; i<[UNIT_DISPLAY count]; i++) {
		NSDateComponents *cdcomps = [calendar components:[Countdown componentsForUnit:i] fromDate:[NSDate dateWithTimeIntervalSince1970:0] toDate:[calendar dateByAddingComponents:[[self currentCountdown] displayComps] toDate:[NSDate dateWithTimeIntervalSince1970:0] options:0] options:0];
		NSDateComponents *cucomps = [calendar components:[Countdown componentsForUnit:i] fromDate:[NSDate dateWithTimeIntervalSince1970:0] toDate:[calendar dateByAddingComponents:[[[self currentCountdown] elapsed] displayComps] toDate:[NSDate dateWithTimeIntervalSince1970:0] options:0] options:0];
		if (!cdcomps || !cucomps || [Countdown componentFromComps:cdcomps withUnit:i] == NSIntegerMax || [Countdown componentFromComps:cucomps withUnit:i] == NSIntegerMax) {
...
- (IBAction)loadCountdownDetailView:(id)sender {
	CountdownDetailViewController *presenting = [self subviewController];
...
- (void)updateUI {
    ...
    [str1 appendFormat:@"Target date: %@", [Countdown stringForDate:[[self currentCountdown] date]]];
    [[self targetDateField] setText:str1];
    
    NSMutableString *str2 = [NSMutableString new];
    [str2 appendFormat:@"Start date: %@", [Countdown stringForDate:[[[self currentCountdown] elapsed] date]]];
    [[self startDateField] setText:str2];
	...
	if ([[actionSheet title] isEqualToString:@"Use which timeset?"]) {
		if (![[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Cancel"]) {
			...
			if ([[self currentCountdown] timeset] != newTimeset) {
				...
				[[[self currentCountdown] elapsed] setTimeset:newTimeset];
				[[self timesetButton] setTitle:[NSString stringWithFormat:@"Timeset: %@", [[[self currentCountdown] timeset] name]] forState:UIControlStateNormal];
				[self updateUI];
                [[self currentCountdown] setupCountdown];
			}
		...
		if (![[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Cancel"]) {
			...
			[[[self currentCountdown] elapsed] setUnit:6-buttonIndex];
			[[self unitButton] setTitle:[NSString stringWithFormat:@"Largest unit: %@", [UNIT_DISPLAY objectAtIndex:[[self currentCountdown] unit]]] forState:UIControlStateNormal];
		}
	}
	else if ([[actionSheet title] isEqualToString:@"Timesets may not be applied to countdowns with more than 100 years remaining until their target dates. Remove timeset from this countdown?"]) {
		if (![[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Cancel"]) {
			Timeset *none;
			for (int i=0; i<[[self timesets] count]; i++) {
			...
			[[[self currentCountdown] elapsed] setTimeset:none];
			[self finishDateSetting1];
			[[self timesetButton] setTitle:[NSString stringWithFormat:@"Timeset: %@", [[[self currentCountdown] timeset] name]] forState:UIControlStateNormal];
		}
	}
	else if ([[actionSheet title] isEqualToString:@"Timesets may not be applied to countdowns with more than 100 years elapsed since their start dates. Remove timeset from this countdown?"]) {
		if (![[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Cancel"]) {
			Timeset *none;
			for (int i=0; i<[[self timesets] count]; i++) {
				if ([[[[self timesets] objectAtIndex:i] self] invincible]) {
...
- (void)retrieveVariables {
    ...
    [Flurry logEvent:@"Viewed_Edit_Countdown"];
	[self setTimesets:[Shared timesets]];
	// END RETRIEVAL
//	[[self timesetButton] setTitleColor:[[self timesetButton] titleColorForState:UIControlStateNormal] forState:UIControlStateSelected];
//	[[self timesetButton] setTitleColor:[[self timesetButton] titleColorForState:UIControlStateNormal] forState:UIControlStateHighlighted];
	[[self timesetButton] setTitle:[NSString stringWithFormat:@"Timeset: %@", [[[self currentCountdown] timeset] name]] forState:UIControlStateNormal];
	[[self unitButton] setTitle:[NSString stringWithFormat:@"Largest unit: %@", [UNIT_DISPLAY objectAtIndex:[[self currentCountdown] unit]]] forState:UIControlStateNormal];
